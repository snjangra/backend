...
class userModel {
    ...
    static async updateTokenById({uid,token}) {
        const { date: nowDate, time: nowTime } = getIndianDateTime();
        const [result2] = await pool.query("UPDATE usr_details SET lt_login_dt = ?,lt_login_time = ? WHERE id = ?", [nowDate,nowTime,uid]);
        const sql = `INSERT INTO usr_tkn_history (user_id, tkn, tkn_status, lt_login_dt, lt_login_time) VALUES (?, ?, ?, ?, ?)`;
        const params = [uid, token, 1, nowDate, nowTime];
        const [result] = await pool.query(sql, params);
        if (result.affectedRows > 0) {
            const sql1 = `INSERT INTO usr_login_history (user_id, login_status, lt_login_dt, lt_login_time) VALUES (?, ?, ?, ?)`;
            const params1 = [uid, 1, nowDate, nowTime];
